...
st.set_page_config(layout="wide", page_title="SAP HCM Migration Tool")

# Remove white space padding at the top
st.markdown("""
    <style>
    .block-container {
        padding-top: 0rem;
...
with st.sidebar:
    selected = option_menu(
        ...
        icons=["house", "bar-chart", "tools"],
        default_index=0
    )

# --- HOME PAGE ---
if selected == "Home":
    st.image("resized_banner.jpg", use_column_width=True)

    st.markdown("""
    <div style='background-color:#e6f0ff;padding:15px;border-radius:10px;margin-top:10px;'>
        <h3 style='text-align:center;'>Pioneering the Future of SAP HCM – From Data-Driven Migrations to Enterprise-Ready Variance Management</h3>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("#### 🚀 Accelerate Your SAP Employee Central Migration")
    st.markdown("Purpose-built platform to automate transformation, validation, and migration of HR data from SAP to SuccessFactors.")
    ...
    cols = st.columns(3)
    for col, (icon, title, desc) in zip(cols, tool_features):
        if os.path.exists(icon):
            with open(icon, "rb") as f:
                img_data = base64.b64encode(f.read()).decode()
            col.markdown(
    ...
    # Blue section
    st.markdown("""
    <div style='background-color:#002b5c;padding:40px;margin-top:30px;border-radius:10px;'>
        <h3 style='color:white;text-align:center;'>🌐 Built for SAP & SuccessFactors</h3>
        <p style='color:white;text-align:center;'>A future-ready migration tool that meets audit, compliance, and automation goals.</p>
        <div style='display:flex;justify-content:space-around;margin-top:30px;'>
            <div style='width:30%;text-align:center;'>
                <h4 style='color:white;'>⚙️ SAP EC Implementation</h4>
                <p style='color:white;'>End-to-end HR data migration from SAP On-Premise to SuccessFactors.</p>
            </div>
            <div style='width:30%;text-align:center;'>
                <h4 style='color:white;'>📊 Data Integrity & Compliance</h4>
                <p style='color:white;'>Custom rules, traceability, audit-ready validation and rollback features.</p>
            </div>
            <div style='width:30%;text-align:center;'>
                <h4 style='color:white;'>📦 Document-Ready Migrations</h4>
                <p style='color:white;'>Generate clean SuccessFactors upload files in one-click with audit trails.</p>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)

# You can now continue the "Solutions" and "Services" sections below as per your earlier structure.
